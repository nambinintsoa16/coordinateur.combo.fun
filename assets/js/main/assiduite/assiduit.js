$(document).ready(function () {
     $(".DataTables").DataTable({
      ...
      $('.prenom').on('click', function (e) {
       ...
       $.post(base_url + 'Assiduite/Time_Lines', { matricule: matricule }, function (data) {
            $.alert({
                title: "",
                content: data,
    ...
    $('.matricule').on('click', function (e) {
        e.preventDefault();
           loding();
        var parent = $(this).parent().parent();
        var matricule = parent.children().first().text();
        $.post(base_url + 'Assiduite/Time_Line', { matricule: matricule }, function (data) {
            $.alert({
                title: "",
                content: data,
                columnClass: 'col-md-12 col-md-offset-8 col-xs-4 col-xs-offset-8',
    ...
    $('.matriculessss').on('click', function(e){
    e.preventDefault();
    var matricule = $(this).text();
    var link = base_url + "Assiduite/TimeLines/?matricule=" + matricule;
    Table.ajax.url(link);
    Table.ajax.reload(); 
    }); 
