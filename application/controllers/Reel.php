...
class Reel extends My_Controller
{
    ...
    public function W1()
    {
        ...
        foreach ($datas as $row) {
            $ca = 0;
            $facture = $this->accueil_model->reel($dateD, $dateF, $row->Prenom);
            ...
            $sub_array[] =  "<a href='#' class='prod1'>" . number_format($ca, 0, ',', ' ') . "</a>";
            $sub_array[] =  "<a href='#' class='prod2'>" . number_format($this->W2(date('Y-m-8'), date('Y-m-13'), $row->Prenom), 0, ',', ' ') . "</a>";
            $sub_array[] =  "<a href='#' class='prod3'>" . number_format($this->W2(date('Y-m-15'), date('Y-m-21'), $row->Prenom), 0, ',', ' ') . "</a>";
            $sub_array[] =  "<a href='#' class='prod4'>" . number_format($this->W2(date('Y-m-22'), date('Y-m-31'), $row->Prenom), 0, ',', ' ') . "</a>";
            $sub_array[] =  "<a href='#' class='prod5'>" . number_format($this->W2(date('Y-m-1'), date('Y-m-31'), $row->Prenom), 0, ',', ' ') . "</a>";
            $data[] = $sub_array;
        }
        $output = array(
            "data" => $data
    ...
    public function W2($dateD, $dateF, $Prenom)
    {
        ...
        $content = "";
        $ca = 0;
        $facture = $this->accueil_model->reel($dateD, $dateF, $Prenom);
        foreach ($facture as $facture) {
            $ca += ($facture->Quantite * $facture->Prix_detail);
        }
        return $ca;
    ...
    public function months($s2)
    {
        ...
        foreach ($datas as $row) {
            ...
            $sub_array[] =  substr($row->Matricule_personnel, 0, 7);
            $sub_array[] =  "<a href='#' class='prod1'>" . number_format($ca, 0, ',', ' ') . "</a>";
            $sub_array[] =  "<a href='#' class='prod2'>" . number_format($this->W2($moi[$s2] . '-08', $moi[$s2] . '-14', $row->Prenom), 0, ',', ' ') . "</a>";
            $sub_array[] =  "<a href='#' class='prod3'>" . number_format($this->W2($moi[$s2] . '-15', $moi[$s2] . '-21', $row->Prenom), 0, ',', ' ') . "</a>";
            $sub_array[] =  "<a href='#' class='prod4'>" . number_format($this->W2($moi[$s2] . '-22', $moi[$s2] . '-31', $row->Prenom), 0, ',', ' ') . "</a>";
            $sub_array[] =  "<a href='#' class='prod5'>" . number_format($this->W2($moi[$s2] . '-01', $moi[$s2] . '-31', $row->Prenom), 0, ',', ' ') . "</a>";
            $data[] = $sub_array;
        }
        $output = array(
            "data" => $data
    ...
    public function produit()
    {
        ...
        $dateD = date($mo[$parametre] . '-01');
        $dateF = date($mo[$parametre] . '-07');
        $facture = $this->accueil_model->detail_produit_reel($dateD, $dateF, $prenom);
        $arrayContent = array();
        $content = "";
        $ca = 0;
        $produit = 0;
    ...
    public function totalpro1()
    {
        ...
        $facture = $this->accueil_model->detail_produit_reels($dateD, $dateF);
        $arrayContent = array();
        $content = "";
        $ca = 0;
        $produit = 0;
